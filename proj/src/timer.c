...
int
timer_get_config(uint8_t timer, uint8_t* st)
{
  /*
  ...
  uint8_t writeback_cmd = TIMER_RB_CMD | TIMER_RB_COUNT; // 0b11100000
  if (timer == 0) {
    port = TIMER2PORT(0);
  ...
  sys_outb(TIMER_CTRL,
           writeback_cmd); // Writing writeback_cmd to TIMER_CTRL addr
...
int
timer_set_freq(uint8_t timer, uint32_t freq)
{
  /*
  ...
  /* freq need to be >=19 because the largest supported counter value is 2^16
  ...
  /* the 'timer_get_config' function already verifies 'timer' range */
  uint8_t timer_conf;
  if (timer_get_config(timer, &timer_conf))
    return 1;
  ...
  /* get least significant bits from 'timer_ival' */
  uint8_t lsb_load = get_LSB(timer_ival);
  /* get most significant bits from 'timer_ival' */
  uint8_t msb_load = get_MSB(timer_ival);
  ...
  /*
   * we tried different initialization modes (only LSB or only MSB) depending on
   * the calculated 'timer_ival' (counter value) but the test failed when doing
   * so. we believe this is due to the fact that lcom_run utility was expecting
   * us to always write both LSB and MSB.
   */
...
int
timer_display_curr_conf(uint8_t timer, enum timer_status_field field)
{
  /*
  ...
  if (field == 0)
    ...
    enum timer_init input_mode_if[] = {
      INVAL_val, LSB_only, MSB_only, MSB_after_LSB
    };
    timer_info.in_mode =
      input_mode_if[st]; // St is always a number between [0, 3]
  }
  else if (field == 2) {
    /* tsf_mode */
    st &= TIMER_OP_MASK;
